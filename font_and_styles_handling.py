...
from utilities.formatting import iter_paragraphs, is_caption, effective_font
...
def check_font_size(document):
    ...
    standard_font_size_heading_1 = 12

    for i, para in enumerate(iter_paragraphs(document)):
        expected_font_size = 11 if is_caption(para) else standard_font_size_body
        for run in para.runs:
            run_font_size = effective_font(run, para, "size", Pt(expected_font_size)).pt
...
def check_font_style(document):
    ...
    for i, para in enumerate(iter_paragraphs(document)):
        for run in para.runs:
            run_font = effective_font(run, para, "name", standard_font)
...
def check_caption_styles(doc):
    ...
    for i, para in enumerate(iter_paragraphs(doc)):
        ...
        if is_caption(para):
            for run in para.runs:
                run_font = effective_font(run, para, "name", standard_font)
                run_size = effective_font(run, para, "size", standard_size)
